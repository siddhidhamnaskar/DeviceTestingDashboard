...
useEffect(()=>{
  ...
  getAllMobiVendDevices(m.SNoutput).then(res=>{
    if(res){
      ...
      // Set the first device as default (now the first in sorted order)
        ...
        {allMobivendDevices.length > 1 && (
         ...
         <table className="table" style={{fontSize:'14px', width: '100%'}}>
                          ...
                          <tr>
                            <th style={{width: '40%', verticalAlign: 'top', padding: '8px'}}>   
                              <div style={{display:'flex',alignItems:'center',gap:'5px'}}>
                                <button style={{backgroundColor:'#ffc107',color:'white',border:'none',padding:'5px 10px',borderRadius:'4px',margin:'2px',fontSize:'12px'}}  onClick={()=>sendToMqttMessageWithResponse(`*QR:${mobivendQrCodes.QrString}#`)} >
                                  SET QR
                                </button>
                              </div>
                            </th>
